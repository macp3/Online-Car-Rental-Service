...
public class PasswordSucRestorationServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(password.equals(confirmPassword)) {
            try {
                ...
                String query = "UPDATE CLIENTS SET PASSWORD = ?, RESTORECODE=null WHERE EMAIL=?";
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            String query = "select RESTORECODE from CLIENTS FULL JOIN HOMEUSER.COMP_ADMINS CA on CLIENTS.EMAIL = CA.EMAIL FULL JOIN HOMEUSER.EMPLOYEES E on CA.EMAIL = E.EMAIL WHERE CLIENTS.EMAIL = ? OR CA.EMAIL = ? OR E.EMAIL = ?";
            PreparedStatement stmt1 = con.prepareStatement(query, Statement.RETURN_GENERATED_KEYS);
            ...
            if (checkIfClientExist.next()) {
                if(Objects.equals(checkIfClientExist.getString(1), codeString) && codeString != null) {
